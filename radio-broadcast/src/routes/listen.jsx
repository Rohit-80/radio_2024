...
function Listen() {
    ...
    window.addEventListener('beforeunload',()=>{
        let lval = localStorage.getItem('listener') || 1;
        localStorage.setItem('listener',lval-1);
    })
    useEffect(() => {
        socketRef.current.on("bufferHeader", (packet) => {
            setBufferHeader(packet);
        })
        let lval = localStorage.getItem('listener') || 0;
        localStorage.setItem('listener',++lval);
        return ()=>{
            let lval = localStorage.getItem('listener') || 1;
        localStorage.setItem('listener',--lval);
        }
